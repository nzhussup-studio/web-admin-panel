...
export default defineConfig(({ mode }) => {
  ...
  const keycloakUrl =
    process.env.VITE_KEYCLOAK_URL || env.VITE_KEYCLOAK_URL || 'http://localhost:8081'
  const keycloakRealm =
    process.env.VITE_KEYCLOAK_REALM || env.VITE_KEYCLOAK_REALM || 'backend-auth-dev'
  const keycloakClientId =
    process.env.VITE_KEYCLOAK_CLIENT_ID || env.VITE_KEYCLOAK_CLIENT_ID || 'frontend-auth-client'
